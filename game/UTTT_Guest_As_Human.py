...
# Define the Host 
host = 'localhost'# Need to put the IP 
port = 12000
...
def main():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while game_not_over:
            ...
            rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) #The line cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) is in the code because OpenCV (cv2) reads images in BGR (Blue, Green, Red) format by default, whereas the hand detection model from MediaPipe (mp.solutions.hands.Hands()) expects the image to be in RGB format (Red, Green, Blue).
